...
class TeamManagementService:
    ...
    async def create_team(
        ...
        oidc_sync_enabled: bool = False, oidc_group_id: Optional[str] = None, oidc_sync_role: str = "member",
    ) -> EmailTeam:
        """Create a new team.
        ...
        try:
            ...
            if existing_inactive_team:
                ...
                existing_inactive_team.oidc_sync_enabled = oidc_sync_enabled
                existing_inactive_team.oidc_group_id = oidc_group_id if oidc_sync_enabled else None
                existing_inactive_team.oidc_sync_role = oidc_sync_role
                ...
                team = EmailTeam(
                    name=name, description=description, created_by=created_by, is_personal=False,
                    visibility=visibility, max_members=max_members, is_active=True,
                    oidc_sync_enabled=oidc_sync_enabled,
                    oidc_group_id=oidc_group_id if oidc_sync_enabled else None,
    ...
    async def update_team(
        ...
        oidc_sync_enabled: Optional[bool] = None, oidc_group_id: Optional[str] = None, oidc_sync_role: Optional[str] = None,
    ) -> bool:
        ...
        try:
            ...
            if oidc_sync_enabled is not None:
                team.oidc_sync_enabled = oidc_sync_enabled
                # Clear group_id when disabling sync
                if not oidc_sync_enabled:
                    team.oidc_group_id = None

            if oidc_group_id is not None:
                team.oidc_group_id = oidc_group_id.strip() or None

            if oidc_sync_role is not None:
                if oidc_sync_role not in ("owner", "member"):
                    raise ValueError("Invalid oidc_sync_role. Must be 'owner' or 'member'")
                team.oidc_sync_role = oidc_sync_role
